// 
// Model 4 - Mean and Variance Bayesian model
//
// ---------------------------------------------------------------------------------------------------------------
//
// Pain experience depends not only on the pain stimulation, but also on the mean and variance of the cue. 
// These influence the uncertainty of the prior distribution of R such that cues with higher sd (modulated by eta) 
// influence the posterior less than precise cues. 
// 
// P(R|X,Z) ~ Noraml(X, beta) * Normal(q, rho + eta*sd)
// 
// ---------------------------------------------------------------------------------------------------------------
...
transformed parameters {
  ...
  for (n in 1:N){
    ...
    mean_p[n] = get_mean(X[n], q[n], beta_n[n], rho_n[n] + eta_n[n]*std[n]);
    std_p[n] = get_std(X[n], q[n], beta_n[n], rho_n[n] + eta_n[n]*std[n]);
  }
}
