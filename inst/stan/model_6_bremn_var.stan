// 
// Model 6 - Variable full Bayesian model
//
// ---------------------------------------------------------------------------------------------------------------
//
...
parameters {
  ...
  //Hyperparameters for mean and standard deviation of population level statistics
  real<lower=20, upper=80> mu_u;
  ...
  real<lower=0> rho_s;
  real<lower=0, upper=50> nu_s;
  real<lower=0, upper=50> beta_s;
  real<lower=0> eta_s;
...
model {
  ...
  //These weak priors sometimes make the simulations more stable
  // beta_u ~ cauchy(0,100);
  // rho_u ~ cauchy(0,100);
  // eta_u ~ cauchy(0,100);
  // mu_u ~ cauchy(50,200);
